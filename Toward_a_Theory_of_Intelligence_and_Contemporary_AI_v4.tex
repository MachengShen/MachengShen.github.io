...
\author{\textbf{Macheng Shen} and \textbf{GPT-5.2 Pro}\\Contact: macshen93@gmail.com\\Website: machengshen.github.io\\Draft for public discussion - March 2026}
\date{}
...
\subsection{9.1 What exactly is a control-sufficient abstraction?}
...
\subsection{9.2 What is actually learned in modern machine learning?}
...
\subsection{9.3 How important is multiscale adaptation?}
...
\subsection{9.4 What makes an abstraction generalize?}
...
\subsection{9.5 What are the joint limits imposed by physical budgets?}
...
\subsection{9.6 Which safety failures are fundamentally closed-loop failures?}
...
\section{Information, generalization, and dissipation}
...
\subsection{Mutual information and generalization}
...
For the present agenda, the interesting next step is not just to repeat the usual contrast between fitting and generalization. It is to ask whether \textit{control-sufficient abstraction} can be given an information-theoretic reading. Perhaps a useful adaptive system is one that does not preserve arbitrary detail, but preserves the information that remains actionable for prediction, intervention, and regulation.

\subsection{Dissipation and the thermodynamics of computation}
...
\subsection{Why mention these two lines together?}
...
\begin{enumerate}
\item what structure a system retains,
\item what it costs to retain and update that structure,
\item and what kinds of closed-loop competence that retained structure makes possible.
\end{enumerate}
...
Welling, M., \& Teh, Y. W. (2011). Bayesian learning via stochastic gradient Langevin dynamics. In \textit{Proceedings of the 28th International Conference on Machine Learning}.
